...
document.getElementById("fname").addEventListener("blur", (e) => {
  validateName(e, "fname");
});
document.getElementById("lname").addEventListener("blur", (e) => {
  validateName(e, "lname");
});
document.getElementById("uname").addEventListener("blur", (e) => {
  validateUsername(e);
});
...
document.getElementById("cpass").addEventListener("blur", (e) => {
  confirmPassword(e);
});
  ...
  if (isValidInput) {
    ...
    if (id === "fname") {
      if (textValue.length == 0) {
        ...
        error.innerText =
          "Name can contain only letters, spaces, hyphens, apostrophes";
      }
    }
    ...
    // error messages for "Last name"
      ...
      if (textValue.length == 0) {
        ...
        error.innerText =
          "Last name can contain only letters, spaces, hyphens, apostrophes";
      }
    }
  }
}
...
function validateUsername(e) {
  ...
  if (isValidInput) {
    ...
    } else if (textValue.startsWith("_") || /^\d/.test(textValue)) {
      // starts with "_" or number
      error.innerText = "Username can not start with a number or an underscore";
    } else if (textValue.endsWith("_")) {
      // ends with "_"
      error.innerText = "Username can not end with an underscore";
    } else {
      error.innerText =
        "Username can contain only letters, numbers, underscores";
    }
  }
}
...
function validateEmail(e) {
  ...
  if (isValidInput) {
    ...
    if (textValue.length == 0) {
      ...
      !/@[a-zA-Z0-9-]+\.[a-zA-Z0-9-]{2,}(\.[a-zA-Z0-9-]+)*$/.test(textValue)
    ) {
...
document.getElementById("sign-up-form").action = "weather.html";
